...
Deploys a single component from a `cloud.component.yml` file:
...
Deploys all components defined in a `cloud.environment.yml` file:

1. **Parses** your `cloud.environment.yml` file
...
current directory for `cloud.component.yml` first, then `cloud.environment.yml`.
...
```bash
# Start from current directory (auto-detect cloud.component.yml)
cldctl up
...
cldctl up -e cloud.environment.yml

# Environment file in a subdirectory
cldctl up -e ./envs/dev.yml -d local

# With variable overrides
cldctl up -e cloud.environment.yml --var stripe_key=sk_test_123
```
