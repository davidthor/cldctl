...
Each key under `resources` is a type-qualified resource key matching the component's `cld.yml`. Each entry maps an IaC resource address to its cloud resource ID.
