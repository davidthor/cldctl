...
description: "Collaboration-focused deployment configurations in arcctl"
---
...
components:
  # Key is registry address, source is version tag
...
2. **Create the environment** - Use `arcctl env create` to initialize
3. **Apply updates** - Use `arcctl env update <name> <config-file>` to deploy/update all components
4. **Manage lifecycle** - Update, scale, and destroy as needed

```mermaid
flowchart LR
    A[environment.yml] --> B[arcctl env update]
    B --> C[Datacenter]
    C --> D[Cloud Resources]
