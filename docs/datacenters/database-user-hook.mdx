...
The database user hook provisions per-consumer database users with scoped permissions. It is triggered by **implicit databaseUser nodes** that are automatically generated when workloads (deployments, functions, cronjobs, or migration tasks) reference a database via `${{ databases.<name>.* }}` expressions.

## How It Works

When a component declares a database and one or more workloads reference it, cldctl automatically injects a `databaseUser` node **between** the database and each consumer. The consumer then depends on the `databaseUser` node instead of the database directly.
...
When no `databaseUser` hook is defined in the datacenter, the databaseUser node **passes through** all outputs from the parent database node unchanged. This means existing datacenters work identically without any changes — zero breaking changes.

### Custom Behavior (Hook Defined)

When a datacenter defines a `databaseUser` hook, the hook can provision per-consumer database users with scoped permissions, returning customized connection strings with per-consumer credentials.
...
- **Implicit creation**: databaseUser nodes are generated automatically by the graph builder — component authors never declare them
...
- **Pass-through default**: When no hook is defined, all parent database outputs are copied unchanged
- **Expression resolution**: `${{ databases.main.url }}` expressions resolve through the databaseUser node's outputs (not the database directly)
