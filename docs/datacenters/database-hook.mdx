...
These outputs are validated at both build time (`cldctl build datacenter`, `cldctl validate datacenter`) and deploy time. Missing required outputs produce a clear error message.
...
| `username` | string | Database username (not all engines require credentials, e.g., Redis) |
...
## Relationship to databaseUser Hook

When a datacenter defines a `databaseUser` hook and a workload references a database, cldctl automatically injects a `databaseUser` node between the database and the consumer. This allows datacenter authors to provision per-consumer database users with scoped permissions via the [databaseUser hook](/datacenters/database-user-hook).

When no `databaseUser` hook is defined, no databaseUser nodes are created — workloads depend directly on database nodes and expressions resolve from the database's outputs.
