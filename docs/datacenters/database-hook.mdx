...
When a workload references a database, cldctl automatically injects a `databaseUser` node between the database and the consumer. This allows datacenter authors to optionally provision per-consumer database users with scoped permissions via the [databaseUser hook](/datacenters/database-user-hook).

When no `databaseUser` hook is defined, the databaseUser node passes through all database outputs unchanged — existing datacenters work identically with zero changes required.
