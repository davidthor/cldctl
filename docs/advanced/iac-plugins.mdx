...
IaC plugins enable datacenter modules to use different Infrastructure-as-Code frameworks. cldctl supports multiple IaC tools, allowing you to choose the best tool for each module.
...
outputs:
  ...
  url:
    value: "postgresql://postgres:${resources.container.environment.POSTGRES_PASSWORD}@localhost:${resources.container.ports[0].host}/postgres"
    sensitive: true
```
