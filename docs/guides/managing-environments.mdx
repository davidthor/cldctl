...
arcctl create environment staging -d aws-production

# Create with a variable file
arcctl create environment staging -d aws-production --var-file ./vars/staging.dcvars
```
...
# Deploy from local source (useful for testing datacenter changes)
...
arcctl deploy component ghcr.io/myorg/web-app:v1.5.0 -e staging --auto-approve
```
...
arcctl shows an execution plan of what will change:
...
arcctl logs -e staging web-app

# Logs from a specific resource type
arcctl logs -e staging web-app/deployment

# Logs from a specific deployment
arcctl logs -e staging web-app/deployment/api
...
arcctl logs -e staging --since 1h
```
...
# Auto-approve for scripted workflows
arcctl destroy component web-app -e staging --auto-approve
```
...
# Auto-approve
arcctl destroy environment staging --auto-approve
```
...
```bash
arcctl destroy datacenter aws-staging
```
...
for env in $(arcctl list environment --output json | jq -r '.[] | select(.name | startswith("preview-")) | .name'); do
  echo "Destroying stale preview: $env"
  arcctl destroy environment "$env" --auto-approve
done
```
...
arcctl deploy component ghcr.io/myorg/app:v2.0.0 -e green --auto-approve

# Verify green is healthy
arcctl get component app -e green

# Switch traffic (update DNS or load balancer)
# Then destroy old blue
arcctl destroy environment blue --auto-approve
```
...
```bash
# Deploy to staging
arcctl deploy component ghcr.io/myorg/app:v1.5.0 -e staging --auto-approve

# Run tests against staging
./scripts/smoke-test.sh staging

# Promote to production
arcctl deploy component ghcr.io/myorg/app:v1.5.0 -e production --auto-approve
```
