...
- **Docker-based**: Databases and deployments run as containers
- **Process-based Functions**: Run dev servers as local processes
...
environment {
  ...
  # Deployments run as Docker containers
  deployment {
    module "container" {
      plugin = "native"
...
```hcl
database {
  when = node.inputs.databaseType == "mongodb"
