...
When you deploy the gateway, arcctl automatically deploys:
...
arcctl deploy ./gateway -e staging \
...
```bash
# Deploy just auth
arcctl deploy ./auth -e staging

# Deploy orders (auto-deploys auth if not present)
arcctl deploy ./orders -e staging
```
