...
## VM-based Workers with Runtime

For cost-effective worker fleets, you can use the `runtime` property instead of Docker images.
The datacenter provisions VMs (EC2, Droplets, etc.) and bootstraps the runtime environment:

```yaml
# cld.yml (dev base - runs locally)
databases:
  queue:
    type: redis:^7
...
```yaml
# cld.prod.yml (production - runs on VMs)
extends: ./cld.yml
...
deployments:
  worker:
    runtime:
      language: node:20
      packages:
        - ffmpeg          # System dependency for media processing
      setup:
        - npm ci --production
...
This is especially useful when workers need system-level dependencies like `ffmpeg` or `imagemagick`
that are easier to install on VMs than in containers.

## Deploying

```bash
# Build (creates both api and worker images)
cldctl component build . -t ghcr.io/myorg/nestjs-worker:v1.0.0
cldctl component push ghcr.io/myorg/nestjs-worker:v1.0.0

# Deploy
cldctl deploy ghcr.io/myorg/nestjs-worker:v1.0.0 -e production \
