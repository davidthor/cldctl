...
Configure automatic migrations that run after database provisioning. Use a top-level `builds` entry for the migration image:
...
The migration task:
- Image is built via a top-level `builds` entry
- Receives `DATABASE_URL` automatically
- Runs before deployments start
- Must exit successfully for deployment to continue
- Alternatively, use `runtime` instead of `image` for process-based execution without Docker
...
# DATABASE_URL is provided by cldctl
...
```bash
# Build and deploy
cldctl component build . -t ghcr.io/myorg/react-hono-app:v1.0.0
cldctl component push ghcr.io/myorg/react-hono-app:v1.0.0

cldctl deploy ghcr.io/myorg/react-hono-app:v1.0.0 -e staging \
...
<CardGroup cols={2}>
  ...
  <Card title="Developer Workflow" href="/guides/developer-workflow">
