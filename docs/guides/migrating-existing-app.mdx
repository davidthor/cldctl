...
Before writing mapping files, you need to know the IaC resource addresses used by the datacenter's hook modules. Use `cldctl inspect datacenter --modules` to discover them directly from the template:
...
```bash
cldctl inspect datacenter ghcr.io/cldctl/aws-ecs:latest
```
</Tip>
...
| 4 | `cldctl inspect datacenter --modules` | Discover IaC resource addresses for mapping files |
