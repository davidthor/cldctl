...
Even though the infrastructure already exists, cldctl needs a component definition to understand **what** the application requires. Create a `cld.yml` in each project.
...
|------|---------|-------------|
| 1 | Write `cld.yml` + `cldctl build component` | Define and package each component |
