...
## Step 2: Register a Datacenter (Without Provisioning)

cldctl needs a datacenter to know **how** resources are provisioned. But since your infrastructure already exists, you don't want the deploy to create new root-level modules (VPC, networking, etc.). Use `--skip-modules` to register the datacenter without provisioning anything:

```bash
# Pull the official AWS ECS datacenter template
cldctl pull datacenter ghcr.io/cldctl/aws-ecs:latest

# Register it — --skip-modules prevents root module provisioning
...
This saves the datacenter state (variables, source reference, component declarations) but does **not** provision any root-level modules. The datacenter is now known to cldctl, so hook matching works for environment-level imports.

### Import Root Modules

If the datacenter template has root-level modules for shared infrastructure (VPC, ALB, networking), import them now so cldctl knows about the existing resources:
...
cldctl import datacenter prod-dc \
  --module alb \
  --map "aws_lb.main=arn:aws:elasticloadbalancing:us-east-1:123456789:loadbalancer/app/acme-alb/abc123"
```

<Note>
If the datacenter template has no root-level modules (some templates only define hooks and rely on pre-existing shared infrastructure), you can skip this step.
</Note>

Later, once all imports are complete, you can run `deploy datacenter` again **without** `--skip-modules` to reconcile. Since the modules already have imported state, cldctl will update them in place rather than creating duplicates.
...
| 3 | `cldctl deploy datacenter --skip-modules` | Register the datacenter without provisioning |
| 4 | `cldctl import datacenter --module` | Import existing root-level modules |
| 5 | `cldctl create environment` | Create an empty environment |
| 6 | Write mapping YAML | Map cloud resource IDs to IaC addresses |
| 7 | `cldctl import component` | Adopt existing resources into state |
